...
-- 4. devuelve la sucursal de donde salio el vehiculo con placa ABC1234
select * from vehiculos;
select * from alquileres;

select s.ciudad from sucursales s inner join alquileres a on s.id = a.id_sucursal inner join vehiculos v on v.id = a.id_vehiculo where v.placa = 'ABC1234';
...
-- 26. crea una que cuente cusntos clientes estan en el sistema
...
-- 27.crea una funcion que convierte el modelo en mayuscula
delimiter //
...
-- 28. crea una funcion que calcule el porecentaje del valos alquiler semanal
select * from descuento;
delimiter //
...
-- 29 .calcula el valor cotizado de todos los alquileres creando una funcion
delimiter //
create function val(valor_cotizado int)
returns int
deterministic
begin
	declare val int;
    select sum(valor_cotiado) into val from alquileres;
    return val;
end //
delimiter ;
select * from alquileres;
select val(valor_cotisado) as valor_cotizado from alquileres;

-- 30 cual es el porcentaje entre el valor de alquiler semanal
delimiter //
create function porcentaje(valor_alquiler_semanal decimal (10,2))
returns decimal 
deterministic
begin
