...
-- 1
delimiter //
create function total(con int)
returns int
deterministic
begin 
	declare total int;
    select count(c.id) into total from cliente c inner join pedido p on p.id_cliente = c.id;
    return total;
end //
delimiter ; 

select distinct total(id) from cliente;
...
-- 3
delimiter //
create function maxi(id int)
returns int
deterministic
begin
	declare maxi int;
    select max(c.id) into maxi from cliente c inner join pedido p on c.id = p.id_cliente;
    return maxi;
end //
delimiter ;

select distinct maxi(id) from cliente;
