...
delimiter //
create procedure newSucursal(ciu varchar(30), tele int, cel int(30), correo varchar(100))
begin
	insert into sucursales(ciudad, telefono_fijo, celular, correo_electronico) values (ciu, tele, cel, correo);
end //
delimiter ;

call newSucursal("tibu", 99886354, 31234455, "tibu@gmail.com");

select * from sucursales;

-- 2 crea un nuevo cliente
delimiter //
create procedure newCli(in cedu int, nom varchar(50), correo varchar(100), dirr varchar(30), resi varchar(30), celu int)
begin
	insert into clientes(cedula, nombre, correo_electronico, direccion, residencia, celular) values (cedu, nom, correo, dirr, resi, celu);
end //
delimiter ;

call newCli(1900754, "jhon", "jhon@gmail.com", "calle miraflores 12", "tibu", 3225677);

select * from clientes;

-- 3 crea un nuevo empleado
delimiter //
create procedure newEmpleado(in cedu int, nom varchar(50), apell varchar(50), celu int,correo varchar(100), ciu_resi varchar(30), sucurr_id int)
begin
	insert into empleado(cedula, nombre, apellido, celular, correo_electronico, ciudad_residencia, id_sucursal) values (cedu, nom, apell, celu, correo, ciu_resi, sucurr_id);
end //
delimiter ;

call newEmpleado(180864, "camila", "martinez", "345343453", "camila@gmail.com", "cucuta", 100);

select * from empleado;

-- 4 crea un nuevo vehiculo
delimiter //
create procedure newVehic(in pla varchar(15), tipo varchar(20), model varchar(30), pue int, capa int,sun varchar(10), mot varchar(50), col varchar(50))
begin
	insert into vehiculos(placa, tipo_vehiculo, modelo, puertas, capacidad, sunroof, motor, color) values (pla, tipo, model, pue, capa, sun, mot, col);
end //
delimiter ;

call newVehic("JFY-23", "electrico", "mazda3", 4, 5, "si", "v8", "rojo");
