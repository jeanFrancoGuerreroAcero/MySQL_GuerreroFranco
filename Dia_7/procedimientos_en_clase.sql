...
delimiter //
create procedure insertarTrabajo(in titulo_trabajo varchar(50), salario_minimo decimal(10,2), maximo_salario decimal(10,2))
begin
	insert into jobs(job_title, min_salary, max_salary) values (titulo_trabajo, salario_minimo, maximo_salario);
end// 
delimiter ;
...
-- 2. actualiza el id de el pais
delimiter //
create procedure actuCountries(in  pais_id varchar(15),  pais_nombre varchar(25), regi_id int)
begin
	if exists(select 1 from countries where country_id = pais_id) then
    
    update countries
    set  country_name = pais_nombre, region_id = regi_id
    where country_id = pais_id ;
    
    select concat('el id ', pais_id , 'actualizada a', pais_nombre ) as mensaje;
    
    else
    
		select 'el id no existe' as mensaje_error;
	end if;
end //
delimiter ;

call actuCountries("CA","colombia",4);
select * from countries;

-- 3 crea una nueva location
select * from locations;
delimiter //
create procedure actuLocations(in address varchar(100),codigo_postal varchar(20), nombre_locations varchar(50), estado_providencia varchar(50), id_pais char(2))
begin
	insert into locations(street_address, postal_code, city, state_province, country_id) values (address, codigo_postal, nombre_locations, estado_providencia, id_pais);
end //
delimiter ;

call actuLocations("567asef", "3456", "colombia", "ME", "CA");
